...
async def upload_document(
    ...
    """
    Upload a PDF to a collection. Returns immediately with status=pending.
    Ingestion (parsing, chunking, embedding) runs asynchronously.
    Week 2: wire up background ingestion task.
    """
    logger.debug("documents.upload.request", collection_id=str(collection_id), filename=file.filename, user_id=str(user_id))
